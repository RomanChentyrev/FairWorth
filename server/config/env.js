...
const schema = z.object({
  ...
  API_RATE_LIMIT: z.coerce.number().int().min(1).default(180),
  ...
  SENTRY_ENVIRONMENT: z.string().trim().min(1).max(64).optional(),
  SENTRY_RELEASE: z.string().trim().min(7).max(200).optional(),
  SENTRY_TRACES_SAMPLE_RATE: z.coerce.number().min(0).max(1).default(0.1),
  SENTRY_PROVIDER_EVENT_INTERVAL_MS: z.coerce.number().int().min(60000).max(86400000).default(300000),
