...
test('search → filter → compare two hotels → details → provider redirect', async ({ page, request }) => {
  ...
  await page.route('**/api/partners/clicks', route => route.fulfill({ status: 201, json: { click_id: 'e2e-click', redirect_url: '/api/partners/redirect/e2e-click' } }));
  ...
  await page.evaluate(() => { window.open = url => { window.__bookingUrl = url; return null; }; });
  await page.getByTestId('continue-at-provider').click();
  await expect.poll(() => page.evaluate(() => window.__bookingUrl)).toBe('/api/partners/redirect/e2e-click');
});
